...
// Email notification function (temporarily disabled)
/* async function sendReportNotification(reportData: any, reportId: string) {
  try {
    // Get OAuth2 credentials from Firebase secrets
    ...
    if (!clientId || !clientSecret || !refreshToken) {
      console.warn(
        "Gmail OAuth credentials not configured. Skipping email notification."
      );
      return;
    }
    ...
    // Format story content for email
    const storyExcerpt = reportData.storyContent
      ? reportData.storyContent
          .slice(0, 2)
          .map(
            (page: any, index: number) =>
              `Page ${index + 1}: ${page.text?.substring(0, 200)}...`
          )
          .join("\n\n")
      : "No content available";

    // Create email content
    const emailSubject = `🚩 Story Report: "${reportData.storyTitle}"`;
    const emailBody = `
A story has been reported for inappropriate content.

📋 REPORT DETAILS:
• Report ID: ${reportId}
• Story ID: ${reportData.storyId}
• Story Title: ${reportData.storyTitle}
• Reported by: ${reportData.reportedByEmail} (${reportData.reportedBy})
• Reported at: ${new Date(reportData.reportedAt).toLocaleString()}

📖 STORY EXCERPT:
${storyExcerpt}

🔧 STORY CONFIGURATION:
• Child: ${reportData.storyConfiguration?.childName || "Unknown"}
• Theme: ${reportData.storyConfiguration?.theme || "Unknown"}
• Mood: ${reportData.storyConfiguration?.mood || "Unknown"}
• Illustrations: ${reportData.storyConfiguration?.enableIllustrations ? "Yes" : "No"}

🔗 ACTIONS:
View full report in Firestore collection: storyReports/${reportId}

---
DreamWeaver App - Story Report System
    `;
    ...
    console.log(
      `Report notification sent to ${ADMIN_EMAIL} for report ${reportId}`
    );
  } catch (error) {
    console.error("Failed to send report notification email:", error);
    // Don't throw error - email failure shouldn't fail the report
  }
} */
