...
export async function generateImagesInBackground(
  ...
  console.log(
    `[generateImagesInBackground] Starting for story ${storyId}, pages: ${storyPages.length}`
  );
  ...
  try {
    ...
    console.log(
      `[generateImagesInBackground] Updating status to 'generating' for story ${storyId}`
    );
    ...
    console.log(
      `[generateImagesInBackground] Using image provider: ${imageProvider}`
    );
    ...
    if (!firstPage.imagePrompt) {
      console.error(
        `[generateImagesInBackground] First page missing image prompt`
      );
    ...
    console.log(
      `[generateImagesInBackground] Generating page 1 image with prompt: ${firstPagePrompt.substring(0, 100)}...`
    );
    ...
    console.log(
      `[generateImagesInBackground] Page 1 image generated successfully: ${firstPageImageUrl.substring(0, 50)}...`
    );
    ...
    if (firstPageImageUrl.startsWith("data:")) {
      ...
      console.log(
        `[generateImagesInBackground] Converting data URL to buffer for upload`
      );
      page1Buffer = dataURLToBuffer(firstPageImageUrl).buffer;
    } else {
      // FLUX returns regular URL
      console.log(
        `[generateImagesInBackground] Fetching page 1 image for upload`
      );
    ...
    try {
      ...
      console.log(
        `[generateImagesInBackground] Uploading page 1 image to Firebase Storage: ${filePath}`
      );
      ...
      console.log(
        `[generateImagesInBackground] Page 1 image uploaded successfully to: ${page1StorageUrl}`
      );
      ...
      console.log(
        `[generateImagesInBackground] Updating Firestore with page 1 image as cover`
      );
      await storyRef.update({
      ...
      console.error(
        `[generateImagesInBackground] Failed to upload page 1 image to storage:`,
        uploadError
      );
      throw uploadError;
    }
    ...
    console.log(
      `[generateImagesInBackground] Starting series generation for ${storyContent.pages.length - 1} remaining pages`
    );
    ...
    for (let i = 1; i < storyContent.pages.length; i++) {
      ...
      if (page.imagePrompt) {
        ...
        console.log(
          `[generateImagesInBackground] Starting generation for page ${pageIndex + 1}`
        );
        ...
        try {
          ...
          if (i > 1) {
            console.log(
              `[generateImagesInBackground] Adding 1s delay before page ${pageIndex + 1}`
            );
          ...
          console.log(
            `[generateImagesInBackground] Page ${pageIndex + 1} prompt: ${subsequentPrompt.substring(0, 100)}...`
          );
          ...
          console.log(
            `[generateImagesInBackground] Page ${pageIndex + 1} image generated: ${tempImageUrl.substring(0, 50)}...`
          );
          ...
          if (tempImageUrl) {
            try {
              ...
              console.log(
                `[generateImagesInBackground] Uploading page ${pageIndex + 1} image to Firebase Storage`
              );
              ...
              console.log(
                `[generateImagesInBackground] Page ${pageIndex + 1} uploaded successfully: ${imageUrl}`
              );
              ...
              // Update Firestore with the new image (update entire storyContent array to maintain array structure)
              console.log(
                `[generateImagesInBackground] Updating Firestore for page ${pageIndex + 1}`
              );
              await storyRef.update({
              ...
              console.log(
                `[generateImagesInBackground] Page ${pageIndex + 1} completed successfully`
              );
            } catch (uploadError) {
              console.error(
            ...
            console.warn(
              `[generateImagesInBackground] Page ${pageIndex + 1}: No image URL returned`
            );
          }
        } catch (error) {
          console.error(
            `[generateImagesInBackground] Failed to generate image for page ${pageIndex + 1}:`,
            error
          );
    ...
    // Update final status
    console.log(
      `[generateImagesInBackground] Series generation completed. Total images generated: ${imagesGenerated}`
    );
    await storyRef.update({
    ...
    console.log(
      `[generateImagesInBackground] Background image generation completed successfully for story ${storyId}`
    );
  } catch (error) {
    console.error(
      `[generateImagesInBackground] Error in background image generation for story ${storyId}:`,
      error
    );
