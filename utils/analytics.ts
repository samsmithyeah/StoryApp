...
export class Analytics {
  ...
  static async logStoryShared(parameters: {
    story_id: string;
    share_method: string;
  }) {
    await this.logEvent('story_shared', parameters);
  }

  static async logCharacterCreated(parameters: {
    character_name?: string;
    age_group?: string;
  }) {
    await this.logEvent('character_created', parameters);
  }

  static async logSettingsChanged(parameters: {
    setting_name: string;
    new_value: string;
  }) {
  ...
  static async logWizardStepEntered(stepName: string, stepIndex: number, fromStep?: string, timeSpentMs?: number) {
  ...
  static async logWizardAbandoned(stepName: string, stepIndex: number, completionPercent: number) {
  ...
  static async logChildProfileEvent(eventType: 'edited' | 'deleted', profileCount: number) {
