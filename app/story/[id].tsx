    ...
    const unsubscribe = onSnapshot(
      ...
      (docSnapshot) => {
        if (docSnapshot.exists()) {
          ...
          if (!story) {
            const storyAge = new Date().getTime() - updatedStory.createdAt.getTime();
            const storyAgeDays = Math.floor(storyAge / (1000 * 60 * 60 * 24));
            
            Analytics.logStoryOpened({
              story_id: updatedStory.id,
              story_length: updatedStory.storyContent.length > 8 ? 'long' : updatedStory.storyContent.length > 4 ? 'medium' : 'short',
              has_illustrations: updatedStory.storyContent.some(page => page.imageUrl),
