...
describe('Auth Security & Edge Cases', () => {
  ...
  describe('Network error simulation', () => {
    const simulateNetworkError = (errorType: 'timeout' | 'offline' | 'server_error') => {
      switch (errorType) {
        case 'timeout':
          return Promise.reject({ code: 'network/timeout', message: 'Request timeout' });
        case 'offline':
          return Promise.reject({ code: 'network/offline', message: 'No internet connection' });
        case 'server_error':
          return Promise.reject({ code: 'network/server-error', message: 'Server unavailable' });
        default:
  ...
  describe('Input boundary testing', () => {
    const validateInputLengths = (input: { email?: string; password?: string; displayName?: string }) => {
      const errors: string[] = [];
      ...
      if (input.displayName) {
        if (input.displayName.length > 100) errors.push('Display name too long');
