...
describe('Auth User Journeys', () => {
  describe('Complete signup flow', () => {
    ...
    it('should complete full signup flow successfully', () => {
      ...
      // Step 1: Start signup
      const signupResult = flow.startSignup('test@example.com', 'password123', 'Test User');
  ...
  describe('Session management', () => {
    const createSessionManager = () => {
      let sessions: { [sessionId: string]: { userId: string; expiresAt: number; refreshToken: string } } = {};
      let currentSessionId: string | null = null;

      return {
        createSession: (userId: string, duration: number = 3600000) => { // 1 hour default
        ...
        refreshSession: (sessionId: string, refreshToken: string) => {
          ...
          sessions[newSessionId] = { userId: session.userId, expiresAt, refreshToken: newRefreshToken };
          delete sessions[sessionId];
          currentSessionId = newSessionId;

          return { success: true, sessionId: newSessionId, expiresAt, refreshToken: newRefreshToken };
        },
    ...
    it('should refresh sessions with valid refresh token', () => {
      ...
      const refreshResult = manager.refreshSession(originalSession.sessionId, originalSession.refreshToken);
    ...
    it('should reject refresh with invalid token', () => {
      ...
      const session = manager.createSession('user123');
      const refreshResult = manager.refreshSession(session.sessionId, 'invalid-refresh-token');
